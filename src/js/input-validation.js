...
export function addIPValidation(input) {
    ...
    input.addEventListener('input', (e) => {
        ...
        if (result.valid === true) {
            ...
            feedback.innerHTML = '<span class="material-symbols-outlined text-green-500 !text-sm">check_circle</span>';
...
export function addHostsValidation(input) {
    ...
    const validateHosts = (value) => {
        ...
        return { valid: true, message: `${hosts.length} subnet${hosts.length > 1 ? 's' : ''}`, count: hosts.length };
    };
...
export function addExampleButton(input, exampleValue) {
    ...
    button.type = 'button';
    button.className = 'absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-primary hover:text-white bg-primary/10 hover:bg-primary px-2 py-1 rounded transition-all font-bold uppercase tracking-wider';
